...
if __name__ == "__main__":
    ...
    for i in indices[:]:
        ...
        y_nn = []
        for dp in i[0]:
            if s['file_range']['./data/artificial/v4/bright_tags_by_trial_MC.csv'][0] <= dp and \
                dp <= s['file_range']['./data/artificial/v4/bright_tags_by_trial_MC.csv'][1]:
                X_nn.append(X[dp])
                y_nn.append(0)

            if s['file_range']['./data/artificial/v4/dark_tags_by_trial_with_decay_MC.csv'][0] <= dp and \
                dp <= s['file_range']['./data/artificial/v4/dark_tags_by_trial_with_decay_MC.csv'][1]:
                X_nn.append(X[dp])
                y_nn.append(1)

        # Train neural network
        pipeline.fit(X_nn, y_nn)
        print(len(X[i[0]]))
        print(len(X[i[1]]))
        print()
